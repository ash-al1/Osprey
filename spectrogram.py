...
def chunkRecv(ringSize=100):
    ...
    fix, ax, img = initSpectroWindow((30, 256))
...
def initSpectroWindow(shape=(30, 256)):
    ...
    Returns:
        tuple: Matplotlib figure, axis and image handle for updating
    """
    fig, ax = plt.subplots()
    freq_range = np.linspace(CENTERFREQ - SAMPLERATE / 2, CENTERFREQ +
                             SAMPLERATE / 2, shape[1])
    time_range = np.arange(shape[0])
    extent = [freq_range[0], freq_range[-1], time_range[0], time_range[-1]]
    img = ax.imshow(np.zeros(shape), aspect='auto', origin='lower',
                    cmap='rainbow', vmin=-100, vmax=0, extent=extent)
    plt.colorbar(img, ax=ax, label='Power (dB)')
    plt.xlabel('Frequency (Hz)')
    plt.ylabel('Time')
    plt.title(f'Spectrogram at {CENTERFREQ} Hz')
    plt.tight_layout()
    plt.ion()
    plt.show()
    return fig, ax, img
